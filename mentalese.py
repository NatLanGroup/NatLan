...
def lvl_3_rule_1(token): #ADJ,ADJ -> ADJ
    for child in token.head.children:
        if child == token:
            return
        if child.pos_ == token.pos_ == (u'ADJ'):
            get_concept(child).set_mentalese(get_concept(token).mentalese,get_concept(child).mentalese,'MC')
...
def lvl_4_rule_1(token): #F(NOUN,ADJ)
    if token.head.pos_ == u'NOUN' and token.pos_== u'ADJ':
    ...
    if token.head.lemma_== u'be' and token.dep_ == u'nsubj':
        for child in token.head.children:
            if child.dep_ == u'attr': #nsubj exist always
                get_concept(token.head).set_mentalese(get_concept(token).mentalese,get_concept(child).mentalese,'C')
                get_concept(token).set_is_processed(True)
                get_concept(child).set_is_processed(True)
                
def lvl_5_rule_3(token): # F(nsubj, acomp)
    if token.head.lemma_== u'be' and token.dep_ == u'nsubj':
        for child in token.head.children:
            if get_concept(child).is_processed == True:
                return
...
def lvl_5_rule_4(token): # F(nsubj, R(prep, %1))
    if token.head.lemma_== u'be' and token.dep_ == u'nsubj':
        for child in token.head.children:
            if get_concept(child).is_processed == True:
                return
