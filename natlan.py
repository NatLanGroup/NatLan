...
def process_testinput (tf):                     # input is the Testinput object
    ...
    for ri in range(len(tf.mentalese)):         # take mentalese items (rows in test input file)
        ...
        if (tf.question[ri]==1):                    # if yes, then on endi we assume a question
            ...
            if gl.args.loglevel>0: gl.log.add_log(("Question answering activation:",gl.WM.branchactiv))
...
gl.VS = branch.Version()    # versions object that keeps track of versions of WM
gl.WM = gl.VS.add_WM()      # create the root WM
gl.log = gl.Logging()
