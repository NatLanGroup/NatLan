...
class Reasoning:
    def __init__(self):
        ...
        self.cd_disabled = {}           # remember noreplace cases.
        ...
        self.general_rules = {}         # store the general-special relationship among rules. general:[specials]
        self.CDuses = {}                # lists reasonuse concepts successfully used for CD reasoning for each WM, in a list of lists

    def do_they_match_for_rule(self, db, wm_index, kb_index):                   # check whether WM content (db) matches in structure a rule in KB
    ...
    def arrange_Imcomb(self,imcomb,new,db):              # re-arrange the rules in imcombined so that general rules go to the end
        nongen = []                                     # holds the non-general rules
        delayed = []                                    # holds the general rules
        for imitem in imcomb:
            if imitem[0] in self.general_rules:         # this rule has a more special version
                delayed.append(imitem)                  # will go to the end of imcombined
            else: nongen.append(imitem)                 # goes at beginning
        imcombin = []                                 # start rebuild imcombined
        for imit in nongen:                             # first rules to add
            imcombin.append(imit)
        for imit in delayed:                            # last rules to add
            imcombin.append(imit)
    #    if new==19: print ("ARRANG imcombin:",imcombin,"general_rules:",self.general_rules,"delayed",delayed)
        return imcombin    
    ...
    def convert_KBrules(self,db,new,enable):           #converts the rule fractions in kb_rules to [imlevel,condition] list
        ...
        new_imcom=self.arrange_Imcomb(imcombined[:],new,db)     # re-arrange the rules in imcombined
        db.cp[new].kb_rules=[]                                  # clear previous content, we add all relevant rules now.
        for rulei in range(len(new_imcom)):                     # copy imcombined to kb_rules and add resoned concept (single condition)
            gl.args.total_reasoncount+=1
            prev_rulenum = len(db.cp[new].kb_rules)             # FIX previous length of rules list
            db.cp[new].kb_rules.append(new_imcom[rulei])        # FIX: we do not add to existing kb_rules content!!
            gl.test.track(db,new,"   MATCHING rule.",gl.args.tr_match,rule=gl.KB.cp[new_imcom[rulei][0]].mentstr)
            if enable==1:#FIX or imcombined[rulei][0] not in db.cp[new].unirule_with:   # enable==0 shows this "new" is parent of an IM concept so reasoning must be skipped
                ...
                db.cp[new].unirule_with.add(new_imcom[rulei][0])   # remember that new was reasoned with this Uni-rule
            #    if gl.d==11 and new==19: print ("CONVERT call add_Reas. new_imcom:",new_imcom,"new:",new,"rulei",rulei,"kb_rules",db.cp[new].kb_rules,"prev_rul",prev_rulenum)
                self.add_ReasonedConcept(db,new,rulei)          #add reasoned concept to WM based on new in db - works for single condition. (FIX? call for newly added rules!!)
    ...
    def same_Reasoned(self,pos):                       # is this concept the same as original inputs ?
        for wmused in gl.WM.cp[pos].wmuse:                      # original conceptgs used for this reasoning
            if wmused>0:
                if gl.WM.rec_match(gl.WM.cp[pos],gl.WM.cp[wmused],[pos,wmused])==1:     #reasoned concept is the same as one in wmuse
                    gl.test.track(gl.WM,pos,"      STOP (SAME_REASONED 1) same in wmuse:"+str(wmused)+" ",gl.args.tr_stop)
                    return 1
   ...
   #         if gl.WM.rec_match(gl.WM.cp[pos],gl.WM.cp[samecon],[pos,samecon])==1:   #reasoned concept is the same
    #            wmu_old = set(gl.WM.cp[samecon].wmuse)
      #          wmu_new = set(gl.WM.cp[pos].wmuse)
       #         if len(wmu_new)>0 and wmu_new!=set([-1]) and wmu_new!=set([-2]) and wmu_old==wmu_new:   # resoning originated from same concept
        #            if gl.d==9: print ("&&&&&&&& SAME REASONED 2 old",samecon,"reasoned",pos)
         #           if gl.args.loglevel>0: gl.log.add_log(("INHIBIT reasoning is same_Reasoned. Same wmuse used. inhibited concept: ",gl.WM.cp[pos].mentstr," wmuse",wmu_new," same old concept",samecon))
          #          gl.test.track(gl.WM,pos,"      STOP (SAME_REASONED 2) same reasoned and same wmused in another concept:"+str(samecon)+" ",gl.args.tr_stop)
        ...
        for kbu in gl.WM.cp[pos].kb_use:                        # KB concepts used for this reasoning
            if kbu in gl.WM.cp[pos].kblink:                     # the concept used in KB is the same as the kblink of this WM concept
                gl.test.track(gl.WM,pos,"      STOP (SAME_REASONED 3) same in kb_use as reasoned conc kblink:"+str(gl.WM.cp[pos].kblink)+" ",gl.args.tr_stop)
    ...
    def set_Mostspecial_Used(self,db,coni,rule):                         # set most_special_used field in db, but db is always WM
        ...
        if noset==0:
            ...
            if kbmo>0 and con.relation!=4:                      # identified in KB.  for C relation most special cannot be set in this way.
            ...
            if kbmo==0 and mostkb>0 and con.relation!=4:        # most special only from KB
            ...
            if kbmo>0 and mostkb>0 and con.relation!=4:         # most special value both from WM and KB    
    ...
    def consolidate_New(self,coni):                 # consolidate known, p etc with earlier WM occurence (also with KB, for kblink only.)
        ...
        if samemax>0:
#            if gl.d==9: print ("CONSOL 1 coni",coni,"wmuse",gl.WM.cp[coni].wmuse)
            gl.WM.consolidate_Specialuse(gl.WM,samemax,gl.WM, coni)       # call consolidation of p etc based on most_special_used
 #       if gl.d==9: print ("CONSOL 2 coni",coni,"wmuse",gl.WM.cp[coni].wmuse)
        ...
        if gl.WM.cp[coni].kblink!=[]:                   # this concept occurs in KB
            gl.WM.consolidate_withKB(gl.WM.cp[coni].kblink[0],coni)     # consolidate p, known etc of the new WM concept (just reasoned) with the KB occurence.
    ...
    def same_inKB(self,db,pos):                             # the concept on pos is present in KB and not needed to be reasoned?
        same_in=False
        if len(db.cp[pos].kblink)>0 and db.cp[pos].wmuse==[-3]:                             # [-3] means only KB is used for reasoning
            if frozenset(db.cp[pos].kb_use) in gl.KB.cp[db.cp[pos].kblink[0]].use_ever:     # in KB the same concept was reasoned using the same KB input
                kbl=db.cp[pos].kblink[:]
                if len(kbl)>0:
                    oldact=set(gl.WM.kbactiv_new)
                    gl.act.activKB_Allchild(gl.KB,kbl[0],kbl[0],gl.WM,2,True)    # activate KB based on the concept that is same in KB. activation round=2.
                    if len(gl.WM.kbactiv_new-oldact)>0:
                        gl.test.track(gl.KB,kbl[0],"   ACTIV (SAME_INKB) KB new activated="+str(gl.WM.kbactiv_new-oldact),gl.args.tr_act,rule="")

                same_in=True
                gl.test.track(db,pos,"      STOP (SAME_INKB): same conc in KB here: "+str(db.cp[pos].kblink[0])+" ",gl.args.tr_stop)
        return same_in

    def correct_Center(self,center):           # correct the start where resoned concepts need be removed
    #    print ("CORRECT ci",gl.WM.ci, sorted(gl.WM.cp[gl.WM.ci].parent,reverse=True),"center",center)
        for par in sorted(gl.WM.cp[gl.WM.ci].parent,reverse=True):
            if par==center: center=center-1
    #    print ("CORRECT 2 ci",gl.WM.ci, sorted(gl.WM.cp[gl.WM.ci].parent,reverse=True),"center",center)
        return center
        ...
        if kbottom >= 1:                                    # this is not based on unknown concept
            ...
            for newparent in nplist:                        # parent list of next concept to add
                ...
                if found == 0:
                    ...
                    gl.WM.cp[gl.WM.ci].wmuse=[]                             # start with wmuse empty
                    gl.WM.set_Use2(gl.WM.ci,conclist[:],kb_use[:],finalconcept)      # set used concepts
                    gl.WM.cp[gl.WM.ci].general=set([])                      # clear general
                    ...
                    is_inhibited = False ; inhib_general = False            # inhibitors
                    if finalconcept==1:
                    #FIX 06.01    gl.WM.cp[gl.WM.ci].kb_use=list(set(gl.WM.cp[gl.WM.ci].kb_use) | set(kb_use))   # concepts used in KB, removing duplications  
                        inhib_general = gl.WM.general_Rule(gl.WM.ci,rule[0],self.general_rules)  # check that the rule used is general and reasoning needs inhibited because earlier we reasoned with more special rule
                        if self.reason_Inhibit(relation,newparent,gl.WM)==1: is_inhibited=True    # other ways to inhibit reasoning
                    ...
                    same_kb = self.same_inKB(gl.WM,gl.WM.ci)                # same concept present in KB and not needed to be reasoned
                    if self.same_Reasoned(gl.WM.ci)==1: same_reason=True    # is thje reasoned concept same as an input?
                    else: same_reason=False
                    same_use = gl.WM.check_Useall(gl.WM,gl.WM.ci,samelist)  # check that the reasoned concept on gl.WM.ci uses the same origin as those in samelist
                    if same_use or same_kb or same_reason or is_inhibited or inhib_general:    # dont make this reasoning
                        center = self.correct_Center(center)                # extend removal to parents immediately preceding
                        while gl.WM.ci>center: gl.WM.remove_concept()       # remove all concepts added in this round
                    else:
                        if finalconcept==1:
                            self.consolidate_New(gl.WM.ci)                  # consolidate known, p etc with earlier WM and KB occurence                             
                            if gl.d>0: print ("FINALADD 8 -------- WM=",gl.WM.this,"here=",gl.WM.ci,gl.WM.cp[gl.WM.ci].mentstr,"p=",gl.WM.cp[gl.WM.ci].p,"parents",gl.WM.cp[gl.WM.ci].parent,"wmuse",gl.WM.cp[gl.WM.ci].wmuse,"kb_use",gl.WM.cp[gl.WM.ci].kb_use,"rule:",rule,"reasonuse",gl.WM.cp[gl.WM.ci].reasonuse)
                            ...
                            gl.test.track(gl.WM,gl.WM.ci,"REASONED concept. used:"+str(conclist)+" from KB:"+str(kb_use)+" final KBuse:"+str(gl.WM.cp[gl.WM.ci].kb_use)+" p="+str(gl.WM.cp[gl.WM.ci].p)+" k="+str(gl.WM.cp[gl.WM.ci].known)+" ",gl.args.tr_reas,rule=gl.KB.cp[rule[0]].mentstr)   # track concept reasoned  in track.txt
                            gl.WM.track_Concept(gl.WM.ci,"New REASONED.")               # concept and rule usage tracking
                            if rule==[0] and gl.WM.cp[gl.WM.ci].kb_use==[] and conclist!=[-3]:  # CD reasoning
                                if gl.WM.this not in self.CDuses: self.CDuses[gl.WM.this]=[]
                                self.CDuses[gl.WM.this].append(conclist)            # note input concepts successfully used for CD reasoning in this paragraph
                            #    print ("FINALADD CDUDES added:",conclist)
                else:                                                   #found==1
    ...
    def reason_Inhibit (self,reasoned_rel,reasoned_concept,pdb):     #check reasoned concept to stop stupid reasoning
        ...
        # inhibit reasoning based on p=inhibit rules
        flatlist=[[reasoned_rel],["P"]]                             # here we assemble a flat version of the new concept to be reasoned
        for rpar in reasoned_parents:                               # all reasoned parents
            if pdb.cp[rpar].relation!=1:                                # not a word
                flat=[]                                                     # flat version of concept parent reasoned
                pdb.pattern_Flat(pdb,flat,rpar)                             # store flat version of parent into flat
                inhib_pattern=pdb.pattern_Inhibit(pdb,flat)                 # compare flattened parent conc with inhibit rule structures
                if inhib_pattern[0]==True: 
                    inhibit=1
                    gl.test.track(pdb,rpar,"      STOP (PATTERN): parent inhibited.",gl.args.tr_stop,rule=inhib_pattern[1])
            else:                                                      # a word as parent
                flat=[1,pdb.cp[rpar].kblink[0]]                        # flattened version - very simple in this case of a word
            pdb.assemb_Flat(flat,flatlist)                             # assemble a flat version of the new concept to be reasoned
        flat=[]
        for flatlevel in flatlist:                                     # put the total flat string together from parents
            flat.extend(flatlevel)
            if flatlevel[-1]!="L": flat.append("L")
        inhib_pattern=pdb.pattern_Inhibit(pdb,flat)                    # now check that the total reasoned concept is inhibited or not
        if inhib_pattern[0]==True: 
            inhibit=1
            gl.test.track(pdb,rpar,"      STOP (PATTERN): reasoned conc inhibited.",gl.args.tr_stop,rule=inhib_pattern[1])
   ...
   #         if gl.d==6: print ("INHIBIT, parents:",reasoned_concept)
        ...
        if reasoned_rel==4 and len(reasoned_parents)==2 :               # C(a,b)
            par0cp = pdb.cp[reasoned_parents[0]]                        # the parent conc
            par1cp = pdb.cp[reasoned_parents[1]]                        # the parent conc
            if par0cp.relation==5:                                      # C(F(x,y),b)
                xcpindex = par0cp.parent[0]                             # first parent of F, thsi is x
                xcp = pdb.cp[xcpindex]
                gen=set()
                for geni in xcp.general:                                # add all the concepts general to parent0 to gen
                    if pdb.name=="WM" and len(pdb.cp[geni].kblink)>0: gen.add(pdb.cp[geni].kblink[0])
                    else: gen.add(geni)
                if pdb.name=="WM" and len(par1cp.kblink)>0 : categ = par1cp.kblink[0]       # parent1, the catgeory
                else: categ = reasoned_parents[1]
                if categ in gen:                                        # the category, b in C(a,b), is general to x in C(F(x,y),b)
                    inhibit=1
                    gl.test.track(pdb,reasoned_parents[0],"      STOP (CD_REDUND) because C relation is known in category="+par1cp.mentstr,gl.args.tr_stop,rule="")
                #    gl.test.track(pdb,reasoned_parents[0],"+++TEST C parent 2 = "+str(reasoned_parents[1])+" general to parent1 = "+str(gen)+" categ="+str(categ),gl.args.tr_stop)
        ...
        gl.args.settimer("reason_907: reason_Inhibit",timer()-s)
        return inhibit

    def check_Induction(self,ndb,new,odb,old,rulei,wmpi,implication,condimap):   # check that if this reasoning is an induction, it is enabled or not
        rule = odb.cp[old].kb_rules[rulei]                          # the rule index in  KB
        matchlist = odb.cp[old].rule_match[rulei][wmpi]             # the list of concepts, which, together with new, match the rule
        print ("CHECK INDUCT 1 new:",ndb.name,new,"old",odb.name,old,"rule",rule[0],"matchlist",matchlist,"implication",implication,"condimap",condimap)
    ...
    def generate_MultiConcept(self,new,old,rulei,wmpi):      # create reasoned concept if condition is AND()
        ...
        if len(gl.WM.cp[old].rule_match[rulei][wmpi])+1 == condicount:   # we have all conditions
            if gl.d==9: print ("GEN MULTICONCEPT new:",new,"old",old,"old rule_match",gl.WM.cp[old].rule_match,"condicount",condicount)
            ...
            if len(condimap)>0:                                          # all new parents successfully created in condimap
                ...
                induct = self.check_Induction(gl.WM,new,gl.WM,old,rulei,wmpi,implication,condimap)    # check that if this reasoning is an induction, it is enabled or not
                if 1==1:                                                # here we check that this is either non-induction or got enabled in check_Induction
    ...
    def manage_Implic(self,ndb,new,clist,kbuse):            # manage the implication, set reasoned_concept
        ...
        if ndb.name=="KB":                                  # the IM is in KB, so reasoned_concept is in KB
            ...
            if len(clist)>0:
                for coni in clist:                          # wm use concepts
                    con=gl.WM.cp[coni]
                    if con.wmuse==[] or con.wmuse[0]<0:     # not itself wmused    
                        if len(con.kblink)==0:              # con only in WM
                            if coni not in gl.WM.cp[reasoned_concept].wmuse:
                                gl.WM.cp[reasoned_concept].wmuse.append(coni)       # then coni goes into wmuse
                        else:                               # kblink exists
                            if len(gl.KB.cp[con.kblink[0]].kb_use)==0:   # kblink does not have kb_use
                                if con.kblink[0] not in kbuse: kbuse.append(con.kblink[0])  # kblink itself will be kbuse
                            else:
                                for kbu2 in gl.KB.cp[con.kblink[0]].kb_use:   # kbuse of kblink
                                    if kbu2 not in kbuse: kbuse.append(kbu2)  # kbuse to add to current kbuse
            if  gl.WM.cp[reasoned_concept].wmuse==[-1]:   # nothing in wmuse
                gl.WM.cp[reasoned_concept].wmuse=[-3]
            gl.WM.cp[reasoned_concept].kb_use=kbuse[:]
    #        if gl.d==8: print ("MANAGE IMPIC 9 reasoned_concept",reasoned_concept,"wmuse",gl.WM.cp[reasoned_concept].wmuse)
    ...
    def generate_IMconcept(self,ndb,new,rule,implication,condition,wmcondi,wdb,condi_p=-1): # handle the rule IM(IM(%1,%2),%2) normal implication. 
        if len(gl.KB.cp[condition].parent)==2:          # IM() has a single condition and a single implication TO DO: multiple implications
            if "%" in gl.KB.cp[gl.KB.cp[condition].parent[0]].mentstr:      # % in IM(%1,%2)
                ...
                #    if gl.d==8: print ("GENIM 1 ndb",ndb.name,"new",new,ndb.cp[new].mentstr,"wdb",wdb.name,"wmcondi",wmcondi,wdb.cp[wmcondi].mentstr,"condi known=",wdb.cp[wmcondi].known)
                    added_inwm=0
                    if condi_p==-1:                     # called from generate_uniconcept: the IM rule is found in WM
                        ...
                        if ndb.name=="KB" and ndb.cp[new].parent[0] not in gl.WM.kbactiv:       # condition not activated
                            condiknow=0                     # disable this reasoning
                        condic = ndb.cp[ndb.cp[new].parent[0]]  # FIX4 the condition
                #        if gl.d==8: print ("GENIM 2 condic",ndb.cp[new].parent[0],"aktiv",gl.WM.kbactiv)
        ...
        #                if gl.d==6: print ("GENIM 5 kp_known",kp_known,"index1 index2",index1, index2)
                    ...
                    #    if gl.d==6: print ("GENIM 6 resoned_conc",reasoned_concept,"db",ndb.name,"in KB:",ndb.cp[ndb.cp[new].parent[1]].mentstr,"WM ci",gl.WM.ci)
                    #    if gl.d==6: print ("GENIM 6.1 WM ci-1 ci",gl.WM.cp[gl.WM.ci-1].mentstr,gl.WM.cp[gl.WM.ci].mentstr)
                    ...
                    #    if condi_p!=-1:                         # if we found the condition, we may not add the implication again
                        if added_inwm>0:
                            matching = gl.WM.search_fullmatch(reasoned_p, gl.WM.cp[reasoned_concept].relation, gl.WM.cp[reasoned_concept].parent,rule,[],gl.WM.cp[reasoned_concept].wmuse[:],gl.WM.cp[reasoned_concept].kb_use[:])  #this is probably ok: we may not reason in case we would have a new p value!!
                        if 0==inhibit and 0==matching :         # in matching, reasoning got inhibited based on unknown concept
                            if added_inwm>0 and gl.WM.cp[gl.WM.ci].known>0:     # reasoned concept added via the IM parent only. 
                                if gl.d>0: print ("REASONED  +------- IM in WM:",gl.WM.this," on index:",gl.WM.ci,gl.WM.cp[gl.WM.ci].mentstr,"ndb",ndb.name,"new",new,"clist",clist,"kbuse",kbuse)
                        ...
                        #        if gl.d==6: print ("GENIM 9 resoned_conc",reasoned_concept,"db",ndb.name,"WM ci",gl.WM.ci)
    ...
    def append_Match(self,new,old,rulenew,ruleold,orulei):  #append "new" to old's rule_match wm pack on orulei
        ...
        if gl.d==9: print ("APPEND_MATCH 9 new:",new,"old",old,"old kb_rules",gl.WM.cp[old].kb_rules,"rule_match",gl.WM.cp[old].rule_match)
    ...
    def manage_Replaced (self,rel,pari,repmap,pix,top,odb,curri):     # calculate replacement disable or enable 
        ...
        if (-1 not in self.replaced) and pari in repmap and pix in self.noreplace[rel]:   # now disable
            if pari not in self.kbreplace:                      # kbreplace would enable this replacement (it is just a WM index for a KB index)
                if rel in gl.args.enable_repl:                  # special enablement, override noreplace
                    if len(self.replaced)>0 and self.replaced[-1] in gl.args.enable_repl[rel]:  # replacement happened on special enablement pos
                        ...
                        if len(self.replaced)>1 and self.replaced[-2] in gl.args.enable_repl[rel]:  # replacement happened on special enablement pos
                            ...
                            self.cd_disabled[pari]=repmap[pari]   # remember disable applied
                ...
                reason=" noreplace "+str(rel)+" "+str(pix)+" "
    ...
    #                if gl.d==4: print ("MANAGE curri",curri,odb.cp[curri].mentstr,"pari",pari,odb.cp[pari].mentstr,"ci",gl.WM.ci,"repmappari:",repmap[pari],gl.WM.cp[repmap[pari]].mentstr)
        ...
        if enab==1 and -1 in self.replaced:                  # reasoning was disabled here
            msg = "   DISABLE (CD) reason due to:"+reason+str(self.noreplace[rel])+" relation:"+str(rel)
    ...
    def adjust_Repmap(self,db,repmap):          # a word was added in WM, so repmap indices and recordparents need to be increased 
        to_inc=[]                               # note here which recordparents items to be increased by 1
        for ritem in repmap:                    # 
            if repmap[ritem]>=db.ci:            # only these WM items are impacted. for <ci the addition of the word happens after the item index.
                parli_i=0
                for parli in self.recordparents:        # search for teh proper recordparents entry
                    parentval_i=0
                    for parentval in parli:             # take all parents (will increase all occurances)
                        if parentval==repmap[ritem]:    # this is the parent valjue that needs to be increased
                            to_inc.append([parli_i,parentval_i])    # note to increase the parent index value
                        parentval_i+=1
                    parli_i+=1
                repmap[ritem]+=1                # adjust the remap item, increase WM index by one
        for inc_item in to_inc:                 # noted cases to increase recordparents
            self.recordparents[inc_item[0]][inc_item[1]] += 1   # increase the parent index value
        if len(to_inc)>0:
            a=1
            #print ("ADJUST 9 new recordparents",self.recordparents)
    ...
    def fix_kbreplace(self,pari):                           # for compund concepts, populate kbreplace
        if gl.KB.cp[pari].relation!=1:                  # FIX
            allkb=True
            for secondp in gl.KB.cp[pari].parent:       # iff all parents of pari are in kbreplace, then pari too !!!
                if secondp not in self.kbreplace: allkb=False
            if allkb==True: self.kbreplace.add(pari)     # remember that pari is an index valid in KB

    def fix_imcount(self,repmap,pari,pix,rel):              # decrease imcount if disabled replacement is occuring
        if gl.d==9: print ("FIXIM 1 repmap",repmap,"kbreplace",self.kbreplace,"pari",pari,"addedconcfor",self.addedconcfor,"rel",rel,"pix",pix)
        if pari in repmap and pix in self.noreplace[rel] and pari not in self.kbreplace:   # parent needs replace, and is disabled !!
    ...
    #    if gl.d==9: print ("VREP 2 convert_Parentlist: recordparents",self.recordparents,"parentlist",parentlist,"curri parent",odb.cp[curri].parent,"repmap",repmap)
    #    if len(parentlist)==2: print ("VREP 2 parentlist 1 2",odb.cp[parentlist[0]].mentstr,odb.cp[parentlist[1]].mentstr)
        for pari in odb.cp[curri].parent:       # parents may need to be replaced7  FIX: repl changhed to curri
            ...
            if odb.name=="KB": self.fix_kbreplace(pari)            # for compund concepts, populate kbreplace
            if gl.d==9: print ("VREP 21 rempap",repmap)
            self.manage_Replaced(rel,pari,repmap,pix,top,odb,curri)           # enable or disable replacement
    #        if gl.d==6: print ("VISIT odb",odb.name,"curri",curri,"rel",rel,"pari",pari,"pix",pix,"repmap",repmap,"kbreplace",self.kbreplace,"disbaled",self.cd_disabled)
            if gl.d==9: print ("VREP 3 pari",pari,"repmap",repmap,"recordparents",self.recordparents,"inkb",inkb,"pix",pix)
            if pari in repmap  and (-1 not in self.replaced) : #       parent need replace and is not disabled
                if gl.d==9: print ("VREP 31 pari",pari,"parentlist",parentlist,"pix",pix,"rel",rel,"kbreplace",self.kbreplace,"recordparents",self.recordparents)
                if (pix not in self.noreplace[rel]) or pari in self.kbreplace or self.just_KBrep(pari,repmap):  # either replacement is OK, or replacement is just a KB index replacement with the WM index
                    parentlist[pix]=repmap[pari]        #replace parent
                    if gl.d==9: print ("VREP 32 pari",pari,"parentlist",parentlist,"repmap",repmap,"recordparents",self.recordparents)
                    repusenow=[]
                    if repcount==0:                 # this is the first parent to be replaced
                        self.imcount+=1             # number of compound concepts to add
                        if gl.d==9: print ("???? +++ VREP 33 pari",pari,"recordparents",self.recordparents,"imcoiunt",self.imcount)
                        ...
                        if gl.d==9: print ("VREP 4 pari",pari,"parentlist",parentlist,"repmap",repmap,"recordparents",self.recordparents)
                        ...
                        if gl.d==9: print ("VREP 5 repmap",repmap,"edit recordparents:",self.recordparents)
                ...
                a=1 #FIX 06.11
                #FIX del inkb[-1]                            # remove the inkb item that was added.
                #if gl.d==9: print ("VREP 6 INKB delete!!! rempap",repmap,"inkb",inkb)
            self.fix_imcount(repmap,pari,pix,rel)  # decrese imcount if disabled replacement is occuring
            if gl.d==9: print ("VISITREP 7 fix imcount: inkb:",inkb,"pari",pari,"repmap",repmap,"recordparents:",self.recordparents)
            pix+=1                                  #index of next pari
        if 1 in inkb:                               # any parent value in recordparents is only valid in KB
            if gl.d==9: print ("VISITREP 8 KB:",gl.KB.ci,"inkb:",inkb,"recordparents:",self.recordparents)
            for pixinkb in range(len(inkb)):
                if inkb[pixinkb]==1:                # this parent position in recordparents is valid in KB, because it was not touched
                    ...
                    if wmval>0: 
                        ...
                        if gl.d==9: print ("VISITREP 9 recordparents edit:",self.recordparents)
        if gl.d==9: print ("VISITREP 10 relationbs",self.recordrel,"repmap",repmap,"parents",self.recordparents)
    ...
    def check_Circular(self,new,old,ndb,odb):        # check that we have no circular reasoning based on D relation. new is the D(x,y) rel.
        circular_enab = True                              # we assume not circular, enabled
        if ndb.name=="WM":                                  # only this case is handled for now
            if odb.name=="WM":
                if new in odb.cp[old].wmuse and ndb.cp[new].relation==3:     # DEL:and odb.cp[old].rule_use==[0]     to reason old, we used CD-reasoning and used new, the same D(x,y) relation we want to use again here
                    circular_enab = False                   # disable circular reasoning
                    msg=" disable (circ 1) the use of D() together with concept that already used it. Disabling conc db="+str(ndb.this)+" conc:"+str(new)+"  old="
                if odb.cp[old].rule_use==[0] and ndb.cp[new].rule_use==[0]:        # both inputs were reasoned via CD reasoning
                    if len(odb.cp[old].reasonuse)==2 and len(ndb.cp[new].reasonuse)==2 :   # case:  same D() used for both new and old
                        common=0
                        if odb.cp[old].reasonuse[0] in ndb.cp[new].reasonuse: 
                            if gl.WM.cp[odb.cp[old].reasonuse[0]].relation==3:      # D() used  
                                common = odb.cp[old].reasonuse[0]                   # same concept used in both inputs
                                disab = odb.cp[old].reasonuse[0]
                        if odb.cp[old].reasonuse[1] in ndb.cp[new].reasonuse: 
                            if gl.WM.cp[odb.cp[old].reasonuse[1]].relation==3:      # D() used 
                                common = odb.cp[old].reasonuse[1]                   # same concept used in both inputs
                                disab = odb.cp[old].reasonuse[0]
                        if common>0:
                            circular_enab = False                                   # disable if same D() used for new and old in a CD reasoning. Now we would just change y back to x in  D(x,y)
                            msg=" disable (circ 2) the use of concepts that already used the same D(). Disabling conc db="+str(gl.WM.this)+" conc:"+str(disab)+"  old="
        if circular_enab == False:
            gl.test.track_double(ndb,new,"   DISABLE (CIRC). new=",odb,old,msg,gl.args.tr_dis)                  # track the DISABLE event 
        return circular_enab
    ...
    def process_CDrel(self,new,old,ndb,odb):             # if new is C or D relation, reason on old
        ...
        if len(ndb.cp[new].parent)==2:
            ...
            if kval>0:                                      # inhibit a result where known==0
                ...
                circ_enab=True
                circ_enab = self.check_Circular(new,old,ndb,odb)    # check that we have no circular reasoning for D() relation
                if isenab and circ_enab and ndb.cp[fromc].mentstr in odb.cp[old].mentstr:       #enabled and fromc somewhere occurs in old:
                    start=gl.WM.ci
                    ...
                    self.imcount=0; self.kbreplace=set(); self.addedcount=0; self.recordrel=[];self.recordparents=[];self.topparents=[];self.repused=[]
                    self.addedconcfor = {}
                    if 1==1:
                        ...
                        #    if gl.d==11 and (new>150 and old>150): print ("CDREL 5 new,old",new,old)
                ...
                if ndb.cp[new].relation==3:                       #D relation: replace parent 0 with 1  (or??)
                    ...
                    isenab=self.isenabled_CDreplace(new,old,ndb,odb,fromc,toc)  #check that reasoning is OK
                    circ_enab = self.check_Circular(new,old,ndb,odb)    # check that we have no circular reasoning
                    if isenab and circ_enab and ndb.cp[fromc].mentstr in odb.cp[old].mentstr:  #enabled and fromc somewhere occurs in old:
                        repmap={}                                   #map to hold pairs of concept replacements
                        self.build_Repmap(fromc,toc,ndb,odb,repmap)     # populate repmap
                        self.imcount=0; self.kbreplace=set(); self.addedcount=0; self.recordrel=[];self.recordparents=[];self.topparents=[];self.repused=[]
                        ...
                        try:
                            ...
                            if len(self.topparents)>0 and (-1 not in self.replaced):    # we have something to reason and not disabled
                                conclist_use=[];kb_use=[]
                                self.build_Conclist(new,ndb,old,odb,conclist_use,kb_use)       # get wmuse field content
                            #    if gl.d==11 and (new>150 and old>150): print ("CDREL 6 new,old",new,old)
                                self.finaladd_Concept(conclist_use[:],kb_use[:],pval,self.recordrel,self.recordparents,[0])  # add the reasoned concepts
                        except:
                            gl.error+=1
    ...
    def process_Anyrel(self,new,old,ndb,odb):                   #if new is any relation, use earlier C or D in old to reason
        if (odb.cp[old].relation==3 or odb.cp[old].relation==4) and len(odb.cp[old].parent)==2:   #C or D rel with 2 parents
            ...
            if (menp1 in ndb.cp[new].mentstr and odb.cp[old].relation==3) or menp2 in ndb.cp[new].mentstr:   # C,D parent mentalese in new  
                ...
                if isfirst: 
                    ...
                    self.process_CDrel(old,new,odb,ndb)                         #perform CD reasoning, old is teh CD rel
    ...
    def enter_RuleMatch(self,new,old,odb,ndb,enable):         # VS:same_list deleted. enable: will cause reasoning be skipped if concept is argument of an IM() relation
        ...
        if ndb.cp[new].mentstr != odb.cp[old].mentstr:
            if (ndb.cp[new].relation==3 or ndb.cp[new].relation==4) and odb.cp[old].relation!=1 and enable==1 and len(ndb.cp[new].parent)==2:
                ...
                if ndb.name=="KB" or self.isnew_CDrel(new) :        #see if this C or D rel is the first occurence, or it is in KB
                    if (menp1 in odb.cp[old].mentstr and ndb.cp[new].relation==3) or menp2 in odb.cp[old].mentstr:    # added a condi for menp1. a parent is present in old mentstr  
                    #    if gl.d==11 and menp1 in odb.cp[old].mentstr: print ("ENTER fura poziban a cserélendő. new",ndb.cp[new].mentstr,"old",odb.cp[old].mentstr)
    ...
    def call_Rulematch (self,newdb,wmkbpos,reason_due):      # call Rulematch for reasopn_due, wm_pos pairs. newdb may be either WM or KB.
        for mold in reason_due:                             # iterate over concepts active and not yet used with wm_pos, both in WM and KB
            ...
            if cold.known>0 and (cold.kb_rules!=[] or cnew.relation in [3,4] or cold.relation in [3,4]):  # old has the rule, or it is C, D relation
    ...
    #            if gl.d==8 and (old==53 or wmkbpos==53): print ("CALL_RULE odb",odb.name,"old",old,"wmkbpos",wmkbpos)
    ...
    def collect_Genrules(self):                             # populate general_rules
        for kb_pos in range(0, gl.KB.cp.__len__()):
            if "%" in gl.KB.cp[kb_pos].mentstr and gl.KB.cp[kb_pos].known>0:  # if a rule, full form (k=2)
                for grul in gl.KB.cp[kb_pos].general:                   # the more general rules if any
                    if grul not in self.general_rules: 
                        self.general_rules[grul]=[]                     # first occurence, add it
                    self.general_rules[grul].append(kb_pos)             # add the more special rule to the list
    ...
    def vs_perform_Reason(self, qflag):                 # perform reasoning on all wm versions
        ...
        self.general_rules={}                               # clear: general rule:a list of special rules
        self.collect_Genrules()                             # populate self.general_rules  
        for liv in gl.VS.wmliv:                                     # all living WMs
            ...
            while len(toprocess)>0: # and gl.WM.ci<200:                                 # DEBUG <200 continue reasoning while there is any concept to process
                gl.test.track (gl.WM,gl.WM.ci,"ACTIVATED in KB: "+str(gl.WM.kbactiv_new),gl.args.tr_act)
                for wm_pos in sorted(toprocess):                         # this round, toprocess does not change
                    if wm_pos>0 and gl.WM.cp[wm_pos].known>0:            # this is in WM
                    ...
                    elif wm_pos<0 and gl.KB.cp[-wm_pos].known>0:        # wm_pos is in KB !
                        kb_pos=-wm_pos
            ...
            #    if counter>1 or len(gl.WM.cp)>50: break  #debug
                counter+=1
